...
const ACT_DAYS_NEEDED     = 3;  // …repartidas en al menos N días DISTINTOS (el servidor fecha cada día → forzar 3 días reales encarece las cuentas falsas)
const ACT_MIN_BEST        = 3000; // …y además debe tener un mejor puntaje real ≥ esto (prueba de juego genuino, no cuentas que "banquean" 1 punto)
const RULES_VERSION       = "2026-07-04"; // versión de las reglas del premio que el jugador acepta al participar
...
CREATE INDEX IF NOT EXISTS idx_accept_user ON rule_acceptances (user_id);
ALTER TABLE users ADD COLUMN IF NOT EXISTS best_score INT NOT NULL DEFAULT 0;
ALTER TABLE prize_participants ADD COLUMN IF NOT EXISTS accepted_rules TEXT;
ALTER TABLE prize_participants ADD COLUMN IF NOT EXISTS accepted_at TIMESTAMPTZ;
`;
...
function makePgStore(pool) {
  ...
  return {
    ...
    async updateBest(uid, score) {
      const r = await q(`UPDATE users SET best_score=GREATEST(best_score,$2) WHERE id=$1 RETURNING best_score`, [uid, score]);
      return r.rows.length ? r.rows[0].best_score : score;
    },
    ...
    async setPool(week, cents) {
      await this.ensureWeek(week);
      await q(`UPDATE prize_weeks SET pool_cents=$2 WHERE week=$1`, [week, cents]);
    },
    ...
    async joinPrize(week, uid, rulesVersion) {
      await q(`INSERT INTO prize_participants (week, user_id, accepted_rules, accepted_at) VALUES ($1,$2,$3,now())
               ON CONFLICT (week, user_id) DO UPDATE SET accepted_rules=EXCLUDED.accepted_rules, accepted_at=now()`, [week, uid, rulesVersion || null]);
      if (rulesVersion) await q(`INSERT INTO rule_acceptances (user_id, rules_version, week) VALUES ($1,$2,$3)`, [uid, rulesVersion, week]);
    },
...
function makeFileStore() {
  ...
  return {
    ...
    async init() {},
    async touchUser() {},
    async updateBest(uid, score) { db.best=db.best||{}; db.best[uid]=Math.max(db.best[uid]||0, score); save(); return db.best[uid]; },
    async upsertDaily(seed, uid, name, score, stars) {
    ...
    async setPool(week, cents) { wk(week).pool_cents = cents; save(); },
    ...
    async joinPrize(week, uid, rulesVersion) {
      ...
      const p = db.prizeParts[week][uid] || { joined: Date.now(), wallet: "" };
      p.accepted_rules = rulesVersion || null; p.accepted_at = Date.now();
      db.prizeParts[week][uid] = p;
      db.acceptances = db.acceptances || []; if (rulesVersion) db.acceptances.push({ uid, v: rulesVersion, week, t: Date.now() });
      save();
    },
...
app.post("/api/score", async (req, res) => {
  try {
    ...
    const userBest = await S.updateBest(user.id, s);
    ...
    if (s > 0) {
      ...
      if (ref) {
        ...
        // 2) Premio: pasa a "activo" solo si el referido demostró juego GENUINO:
        //    N partidas válidas en N días DISTINTOS (fechados por el servidor) + un mejor puntaje real.
        //    Esto encarece mucho las cuentas falsas creadas para inflar el desbloqueo del premio.
        if (!ref.active && act.runs >= ACT_RUNS_NEEDED && act.days >= ACT_DAYS_NEEDED && userBest >= ACT_MIN_BEST) {
...
app.post("/api/prize/join", async (req, res) => {
  try {
    const { initData, accept } = req.body || {};
    const user = verifyInitData(initData);
    if (!user) return res.status(401).json({ error: "auth" });
    if (accept !== true) return res.status(400).json({ error: "accept" }); // debe aceptar las reglas explícitamente
    const wkk = weekKey();
    await S.ensureWeek(wkk);
    await S.touchUser(user.id, displayName(user));
    await S.joinPrize(wkk, user.id, RULES_VERSION);
    res.json({ ok: true, week: wkk, rulesVersion: RULES_VERSION });
  } catch (e) { res.status(500).json({ error: "server" }); }
});
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    if (u.message && u.message.text) {
      ...
      } else if (/^\/poolset/.test(txt) && isOwner) {
        const m = txt.match(/^\/poolset\s+([0-9]+(?:[.,][0-9]{1,2})?)/);
        const wkk = weekKey();
        await S.ensureWeek(wkk);
        if (m) {
          const cents = Math.round(parseFloat(m[1].replace(",", ".")) * 100);
          if (cents >= 0 && cents <= 100000000) {
            await S.setPool(wkk, cents);
            const row = await S.getWeekRow(wkk);
            await tg("sendMessage", { chat_id: chat.id, text: `✅ Pozo FIJADO en ${centsToUsdt(cents)} USDT.\nSemana ${wkk}: ${centsToUsdt(row.pool_cents + row.rollover_cents)} USDT (base ${centsToUsdt(row.pool_cents)} + acumulado ${centsToUsdt(row.rollover_cents)})` });
          } else {
            await tg("sendMessage", { chat_id: chat.id, text: "Monto inválido. Ej: /poolset 25 · Reiniciar: /poolset 0" });
          }
        } else {
          await tg("sendMessage", { chat_id: chat.id, text: "Fija el pozo EXACTO (lo pisa, no suma).\nEj: /poolset 25 · Reiniciar: /poolset 0" });
        }
...
/* Política de privacidad pública (para BotFather / tiendas / enlaces). */
...
<title>NERVE — Privacy Policy / Política de Privacidad</title>
<style>body{margin:0;background:#070B1E;color:#EAF0FF;font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;line-height:1.6}
.w{max-width:760px;margin:0 auto;padding:32px 22px 64px}h1{font-size:24px}h2{font-size:18px;margin-top:30px;color:#22D3EE}
h3{margin-top:26px;color:#FBBF24}p,li{color:#C7CFE6;font-size:15px}a{color:#22D3EE}.upd{color:#9AA6C0;font-size:13px}hr{border:none;border-top:1px solid rgba(154,166,192,.2);margin:34px 0}</style></head>
<body><div class="w"><h1>NERVE — Privacy Policy</h1><p class="upd">Last updated: ${RULES_VERSION}</p>
<p>NERVE ("the game") is a Telegram Mini App. This policy explains what we collect and why.</p>
<h2>Data we process</h2><ul>
<li><b>Telegram account data</b> provided by Telegram when you open the game: your Telegram user ID and public first name/username. We do not receive your phone number or contacts.</li>
<li><b>Gameplay data</b>: your scores, best results, daily/weekly rankings and in-game activity.</li>
<li><b>Referrals & prize data</b>: who invited you, your prize participation, and — only if you choose to enter and provide it — your TON wallet address to receive prizes.</li>
<li><b>Technical data</b>: limited request data (e.g. IP address) used temporarily to prevent abuse and rate-limit the service.</li></ul>
<h2>Why we use it</h2><ul><li>To run the game, leaderboards and the weekly prize.</li><li>To prevent cheating and fraud.</li><li>To pay prizes to winners who provide a wallet.</li></ul>
<h2>Sharing</h2><p>We do not sell your data. Telegram operates the platform and ads are served by AdsGram under their own terms. We share data only as needed to operate the game or if required by law.</p>
<h2>Retention & your rights</h2><p>We keep data while you use the game. You can request deletion of your data by contacting us. Deleting the Telegram chat and stopping play also ends new data collection.</p>
<h2>Children</h2><p>The game is not directed to children under the age required in your country, and money prizes are only for users of legal age where such contests are permitted.</p>
<h2>Contact</h2><p>Questions or data requests: message the bot <b>@nerve_game_comercial_bot</b> on Telegram.</p>
<hr><h1>NERVE — Política de Privacidad</h1><p class="upd">Última actualización: ${RULES_VERSION}</p>
<p>NERVE ("el juego") es una Mini App de Telegram. Esta política explica qué datos tratamos y para qué.</p>
<h3>Datos que tratamos</h3><ul>
<li><b>Datos de tu cuenta de Telegram</b> que Telegram nos entrega al abrir el juego: tu ID de usuario y tu nombre/usuario público. No recibimos tu número de teléfono ni tus contactos.</li>
<li><b>Datos de juego</b>: tus puntajes, mejores resultados, rankings diarios/semanales y actividad.</li>
<li><b>Referidos y premios</b>: quién te invitó, tu participación en el premio y — solo si eliges participar y la proporcionas — tu dirección de wallet TON para cobrar premios.</li>
<li><b>Datos técnicos</b>: datos limitados de la conexión (p. ej. dirección IP) usados de forma temporal para evitar abusos y limitar la tasa de uso.</li></ul>
<h3>Para qué los usamos</h3><ul><li>Para operar el juego, los rankings y el premio semanal.</li><li>Para prevenir trampas y fraude.</li><li>Para pagar los premios a los ganadores que proporcionen una wallet.</li></ul>
<h3>Con quién los compartimos</h3><p>No vendemos tus datos. Telegram opera la plataforma y los anuncios los sirve AdsGram bajo sus propios términos. Solo compartimos datos si es necesario para operar el juego o si la ley lo exige.</p>
<h3>Conservación y tus derechos</h3><p>Conservamos los datos mientras uses el juego. Puedes solicitar la eliminación de tus datos contactándonos. Borrar el chat de Telegram y dejar de jugar también detiene la recolección de nuevos datos.</p>
<h3>Menores</h3><p>El juego no está dirigido a menores de la edad requerida en tu país, y los premios en dinero son solo para usuarios con la edad legal donde estos concursos están permitidos.</p>
<h3>Contacto</h3><p>Consultas o solicitudes de datos: escribe al bot <b>@nerve_game_comercial_bot</b> en Telegram.</p>
</div></body></html>`);
});
